...
def constrastLimit(image):
    ...
    channels = list(cv2.split(img_hist_equalized))   # convert tuple → list
...
def findLargestSign(image, contours, threshold, distance_threshold):
    ...
    for c in contours:
        ...
        if len(c.shape) == 2:
            c = c.reshape(-1, 1, 2)
        elif len(c.shape) == 3 and c.shape[1] != 1:
            c = c[:, :1, :]
        ...
        if not is_sign or max_distance < distance_threshold:
            continue

        pts = c.reshape(-1, 2)
        left, top = np.min(pts, axis=0)
        right, bottom = np.max(pts, axis=0)

        area = math.pi * (max_distance ** 2)

        if area > max_area:
            max_area = area
            # Popravljeno: (x, y) format za cv2.rectangle
            best_coordinate = [(left - 3, top - 3), (right + 3, bottom + 3)]
            best_sign = cropContour(image, [cX, cY], max_distance)

    return best_sign, best_coordinate


def findSigns(image, contours, threshold, distance_threshold):
    signs = []
    coordinates = []
    ...
    for c in contours:
        ...
        if is_sign and max_distance > distance_threshold:
            ...
            left, top = np.min(pts, axis=0)
            right, bottom = np.max(pts, axis=0)
            coordinates.append([(left - 2, top - 2), (right + 1, bottom + 1)])
...
def localization(image, min_size_components, similitary_contour_with_circle, model, count, current_sign_type):
    ...
    contrast = constrastLimit(image)
    cv2.imshow("Contrast", contrast)

    LoG = LaplacianOfGaussian(image)
    cv2.imshow("LoG", LoG)

    binary = binarization(LoG)
    cv2.imshow("Binary", binary)

    mask = remove_other_color(image)
    cv2.imshow("Color Mask", mask)

    binary_and_mask = cv2.bitwise_and(binary, binary, mask=mask)
    cv2.imshow("Binary AND Mask", binary_and_mask)
